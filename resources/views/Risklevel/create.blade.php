...
<div class="card">
    ...
    <div class="card-body">
    <form action="{{ route('riskLevel.store') }}" method="POST">
        @csrf
      <div class="d-flex row">
        <div class="mb-4 col col-md-3 mx-3 px-1">
            ...
            <select name="risk_level_audit" id="risk_level_audit" class="form-select" placeholder="Insert Kode Kegiatan" aria-label="Default select example">
                ...
                @foreach ($modelauditee as $dataauditee)
                    <option name="risk_level_audit" value="{{ $dataauditee->id_auditee }}">{{ $dataauditee->nama_auditee }}</option>
                @endforeach
            </select>
        </div>
        ...
        <table class="table" id="datatable">
            ...
            <tbody>
                <tr>
                    <td colspan="2">      
                        <select id="risk_level_risk_kriteria" name="risk_level_risk_kriteria" class="form-select" aria-label="Default select example">
                            <option value="" selected disabled>Insert Risk Kriteria</option>
                            @foreach ($Modelkriteria as $datakriteria)
                                <option value="{{ $datakriteria->id_risk }}">{{ $datakriteria->nama_risk }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td colspan="2">      
                        <select id="risk_level_kegiatan" name="risk_level_kegiatan" class="form-select" placeholder="Insert Kode Kegiatan" aria-label="Default select example">
                            ...
                            @foreach ($modelKegiatan as $dataKegiatan)
